 ...
 *      $Revision: 1.29 $       $Date: 2010/06/23 06:39:12 $
 *
 ***************************************************************************/
...
static void *open_molden_read(const char *filename,
  ...
  while (fgets(buffer,1024,data->file)) {
    ...
    if (!strcmp(keystring, "[5D]")) {
      printf("moldenplugin) [5D] basis found. Currently VMD handles only basis sets with 6D functions.\n");
      return NULL;
    }
    ...
    if (!strcmp(keystring, "[ATOMS]")) {
      ...
      while (buffer[0]!='[' && s!=NULL && !iswhiteline(buffer)) {
        (*natoms)++;
        prevline = ftell(fd);
        s = fgets(buffer, 1024, fd);
      }
  ...
  for (i=0; i<data->numatoms; i++) {
    ...
    while (1) {
      ...
      for (j=0; j<numprims; j++) {
        int nr;
        float expon=0.f, coeff1, coeff2=0.f;
        if (!fgets(buffer,1024,data->file)) return FALSE;
        nr = sscanf(buffer,"%f %f %f", &expon, &coeff1, &coeff2);
        if (nr<2) {
...
static int read_molecular_orbitals(qmdata_t *data) {
  ...
  if (!data->qm_timestep) return FALSE;
